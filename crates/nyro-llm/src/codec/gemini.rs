...
fn content_parts(content: Option<&Content>, images: bool) -> Result<Vec<w::Part>, CodecError> {
    match content {
        ...
        Some(Content::Parts(parts)) => parts
            .iter()
            .map(|p| match p {
                ContentPart::GeminiText(p) => thinking::wire_text(p),
                ContentPart::Text {
                    anthropic_cache_control: None,
                    text,
...
fn check_part(p: &w::Part) -> Result<(), CodecError> {
    let payloads = usize::from(p.text.is_some())
        + usize::from(p.inline_data.is_some())
        + usize::from(p.function_call.is_some())
        + usize::from(p.function_response.is_some());
    if payloads > 1 || (payloads == 0 && p.thought_signature.as_deref().is_none_or(str::is_empty)) {
        return Err(bad(
            "part requires one supported payload or an opaque signature",
        ));
    }
    if thinking::annotated(p) && (p.inline_data.is_some() || p.function_response.is_some()) {
        return Err(bad("thinking metadata on this payload is unsupported"));
    }
    Ok(())
}
fn call(
...
pub fn decode_chat(value: Value, model: &str, streaming: bool) -> Result<ChatRequest, CodecError> {
    ...
    if let Some(s) = wire.system_instruction {
        ...
        for p in s.parts {
            check_part(&p)?;
            if thinking::annotated(&p) {
                return Err(bad("system instructions cannot carry thoughts"));
            }
    ...
    for c in wire.contents {
        ...
        for p in c.parts {
            ...
            if thinking::annotated(&p) && role != Role::Assistant {
                return Err(bad("thinking metadata requires model role"));
            }
...
pub fn encode_chat(r: &ChatRequest) -> Result<Value, CodecError> {
    ...
    wire.generation_config = Some(w::GenerationConfig {
        thinking_config: r.gemini_thinking.clone(),
        temperature: g.temperature,
...
impl StreamDecoder {
    ...
    fn push_inner(
        ...
        let candidates = w.candidates.unwrap_or_default();
        if candidates.len() > 1 {
            return Err(bad("unexpected stream candidates"));
        }
        let mut deltas = vec![];
        if let Some(c) = candidates.into_iter().next() {
            if self.terminal || c.index.unwrap_or(0) != 0 {
            ...
            if let Some(content) = c.content {
                ...
                for p in content.parts {
                        ...
                        || (p.function_call.is_none() && p.thought_signature.is_some())
                    {
                        ...
                        let payload = PartDelta::ToolCall(ToolCallDelta {
                            gemini,
                            index: self.tools,
                            id: Some(id),
                    ...
                    self.parts = self
                        .parts
                        .checked_add(1)
                        .ok_or_else(|| bad("too many stream parts"))?;
                    deltas.push(delta);
                }
            }
            let finish = c
        ...
        if chunk.choices.is_empty() && w.usage_metadata.is_none() {
            return Err(bad("empty Gemini stream event"));
        }
        let usage = w.usage_metadata.map(usage).transpose()?;
        ...
        let per_part = std::mem::size_of::<ChatChunk>()
            ...
            + chunk.model.len()
            + chunk.object.len();
        if deltas.len().saturating_mul(per_part) > self.max_bytes {
            return Err(bad("Gemini stream part expansion exceeds limit"));
        }
        let mut out = vec![];
        // Observe the received frame usage before any part can fail downstream encoding.
        // Finish remains on the last part so no output is emitted after the terminal marker.
        if deltas.is_empty() {
            chunk.usage = usage;
            out.push(ChatEvent::Chunk(Box::new(chunk)));
        } else {
            let last = deltas.len() - 1;
            for (i, delta) in deltas.into_iter().enumerate() {
                let mut part = chunk.clone();
                part.choices[0].delta = delta;
                if i == 0 {
                    part.usage = usage.clone();
                }
                if i != last {
                    part.choices[0].finish_reason = None;
                }
                out.push(ChatEvent::Chunk(Box::new(part)));
            }
        }
        Ok(out)
    }
...
pub struct StreamEncoder {
    ...
    text_position: Option<(StreamPosition, PositionOwner)>,
    bytes: usize,
    max_bytes: usize,
    terminal: bool,
    done: bool,
...
impl StreamEncoder {
    ...
    fn push_inner(&mut self, event: &ChatEvent) -> Result<String, CodecError> {
        ...
        for choice in &c.choices {
            ...
            if choice.delta.events.len() > 1 {
                // Bound the new batch before cloning parts or serializing wire JSON.
                // A single event retains the existing per-payload/frame limits.
                let bytes = choice.delta.events.iter().fold(
                    choice
                        .delta
                        .events
                        .len()
                    ...
                    |bytes, event| {
                        bytes.saturating_add(match &event.delta {
                            ...
                            PartDelta::GeminiText(text) => {
                                text.text.as_ref().map_or(0, String::len).saturating_add(
                                    text.thought_signature.as_ref().map_or(0, String::len),
                                )
                            }
                            _ => 0,
                        })
                ...
                if bytes > self.max_bytes {
                    return Err(bad("Gemini event batch exceeds limit"));
                }
            }
            let mut parts = vec![];
            for event in &choice.delta.events {
                ...
                let owner = match &event.delta {
                    ...
                    PartDelta::ToolCall(_) => None,
                    _ => return Err(bad("invalid stream choice")),
                };
                if let Some(owner) = owner {
                    if let Some((previous, previous_owner)) = self.text_position {
                        let compatible = match (previous.item, event.position.item) {
                            (StreamItem::Ordered(_), StreamItem::Ordered(_)) => {
                                event.position >= previous
                            }
                            (StreamItem::OpenAiMessage, StreamItem::OpenAiMessage) => {
                                event.position == previous
                            }
                            _ => false,
                        };
                        if !compatible
                            || (previous.item == event.position.item && previous_owner != owner)
                        {
                            return Err(bad("text stream position regressed or changed ownership"));
                        }
                    }
                    self.text_position = Some((event.position, owner));
                }
                match &event.delta {
                    PartDelta::GeminiText(part) => {
                        if choice.delta.events.len() != 1 || !self.calls.is_empty() {
                            return Err(bad(
                                "Gemini text/signature part must precede tool calls and remain separate",
                            ));
                        }
                        parts.push(thinking::wire_text(part)?);
                    }
                    PartDelta::Text(text) => {
                    ...
                    PartDelta::ToolCall(d) => {
                        if self.text_position.is_some_and(|(text, _)| matches!(
                            (text.item, event.position.item),
                            (StreamItem::Ordered(text), StreamItem::Ordered(tool)) if tool <= text
                        )) || self.calls.iter().any(|(index, call)| {
                            *index != d.index && call.position == Some(event.position)
                        }) {
                            return Err(bad("tool stream position already belongs to another item"));
                        }
                        ...
                        if self.bytes > self.max_bytes
                            || self.calls.len() >= self.max_bytes
                                && !self.calls.contains_key(&d.index)
                        {
                        ...
                        if call
                            .position
                            .as_ref()
                            .is_some_and(|position| position != &event.position)
                        {
                            return Err(bad("tool call changed stream position"));
                        }
                        call.position = Some(event.position);
                        if let Some(meta) = &d.gemini {
        ...
        if data.len() > self.max_bytes {
            return Err(bad("Gemini stream frame exceeds limit"));
        }
        Ok(format!("data: {data}\n\n"))
    }
}
