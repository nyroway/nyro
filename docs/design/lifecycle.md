...
- **Phase 1**:落地五阶段 `PhaseHook` + `PhaseOutcome` + `PhaseCtx`,按五阶段重排 `dispatch_pipeline` 原生步骤;统一 manifest 与 admin "已加载扩展" 只读视图。
  ...
  - **OnResponse / OnLog 插点归 P2 — 已全部交付**:`ResponseStats` 入 ctx(P2-A1)、`OnLog` 终态插点(P2-A2)、`OnResponse` 非流式 Full + 流式 per-chunk(P2-A3)均已接线。五阶段插点(OnRequest / OnAccess / OnUpstream / OnResponse / OnLog)在 `dispatch_pipeline` 全部就位。
  - **端到端验证 ✅**:`dispatcher` 测试模块内 `#[cfg(test)]` 覆盖三处插点 —— `OnRequest` 哨兵 hook(触发 `ShortCircuit`,断言路由查找前短路返回 200,无路由本应 404)、`OnLog` 探针 hook(断言终态在管线边界触发)、`OnResponse` Full 改写 hook(经 `run_phase_hooks_slice` 断言可原地改写响应体)。示例 hook 仅测试构建注册/驱动,生产与集成测试二进制零影响。
- **Phase 2**:补齐流式 `OnResponseHook`(`AiStreamDelta` per-chunk)与示例扩展(限流 / 语义缓存);按需引入 Hook 链优先级。
  - **P2-A3 ✅ 已交付(OnResponse 插点接线,纯新增)**:`run_phase_hooks` 拆出 `run_phase_hooks_slice`(接受预解析的 `&[&Arc<dyn PhaseHook>]`,供流式逐块复用);`dispatch_pipeline_inner` 把 `ProviderCtx` 影子改名 `provider_ctx`,使 `RequestContext`/`AiRequest`/`HostContext` 可透传进 handler。**非流式**(`handle_non_stream`、force-stream 的 `handle_non_stream_via_upstream_stream`)在 `AiResponse` 构建后、编码前跑 `OnResponse`(`ResponseView::Full`),支持整形 / `ShortCircuit` / `Reject`(替换响应时跳过原生成功日志,OnLog 仍在边界触发)。**流式**(IR round-trip)在 spawn 内逐 `AiStreamDelta` 跑 `OnResponse`(`ResponseView::Stream`,只改写,忽略短路 / 拒绝);仅当存在 OnResponse 钩子时才把 `RequestContext`/`AiRequest`/`Gateway` 克隆进任务(满足 `'static`+`Send`),否则零分配跳过。**字节级 SSE passthrough 路径不接 OnResponse**(无 IR 往返,与非流式 passthrough 一致)。新增 `#[cfg(test)]` Full 改写探针单测;`cargo test -p nyro-core` 全绿(145 lib + 全集成)、`clippy -p nyro-core --all-targets` 零告警。
  - **P2-A2 ✅ 已交付(OnLog 终态插点接线,纯新增)**:`dispatch_pipeline` 抽出 `dispatch_pipeline_inner`(`ctx`/`request` 按 `&mut` 贯穿、`HostContext` 入参、`ProviderCtx` 仍在循环内遮蔽),公开 `dispatch_pipeline` 变薄包装,在 inner 返回**单一汇聚点**跑 `OnLog`。`OnLog` 为终态 fire-and-forget(`ResponseView::Pending`,只观察不改/不短路),读 `ctx.extensions` 的 `ResponseStats` 快照;空注册表零分配 no-op,生产行为中性。新增 `#[cfg(test)]` OnLog 探针 hook + 单测断言其在管线边界触发。
  - **P2-A1 ✅ 已交付(ResponseStats 入 ctx,纯新增)**:`[plugin/phase.rs](../../crates/nyro-core/src/plugin/phase.rs)` 定义 `ResponseStats{client_status, upstream_status, usage, upstream_latency_ms, ttfb_ms, stream_chunks}`(`plugin` 导出)。`dispatch_pipeline` 在 `ProviderCtx` 遮蔽 `RequestContext` 前克隆捕获共享扩展袋,经 `CallCtx.req_ext` 透传到所有 handler;`LogBuilder` 增 `ext` 字段,`from_ctx` 填充、`from_dispatch` 置 `None`,`emit()` 单点把终态指标镜像写入 ctx 的 `ResponseStats` 快照(所有 handler 走 `from_ctx`,一处注入即全覆盖)。落库 `LogEntry` 口径不变,行为中性;新增单测断言注入生效,`cargo test -p nyro-core` 全绿(144 lib + 全集成)、`clippy -p nyro-core --all-targets` 零告警。
