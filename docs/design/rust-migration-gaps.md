...
审计基线：`d943c174`（2026-09-09，含 PR #323）。后续关闭状态：G01 已在本轮模型发现实现中补齐，其余条目仍按审计基线跟踪。本文核对仓库实现和测试，不代表真实厂商或 SDK 兼容认证。[架构文档](architecture.md)仍是目标设计，[实验性代理指南](../standalone/rust-proxy_CN.md)说明当前支持范围。
...
| G02 同协议保真 | 缺失 | [旧调度器](../../crates/nyro-core/src/proxy/dispatcher/mod.rs)按条件选择 Native 模式；[请求构建](../../crates/nyro-core/src/provider/common/pipeline.rs)和[非流式响应](../../crates/nyro-core/src/proxy/dispatcher/non_stream.rs)可跳过 IR 往返。[新 driver](../../crates/nyro-llm/src/provider.rs)始终经过类型化 codec。 | 显式定义同协议兼容能力，包括请求／响应扩展、模型重写、Header 策略和流终态；认证、准入、计量、清理仍必须执行。不能全局移除严格解码或转发全部调用方 Header。旧 builder 测试也不能单独证明端到端保真。 |
| G03 推理、缓存与媒体语义 | 部分 | [旧转换测试](../../crates/nyro-core/tests/protocol_conversion.rs)覆盖 thinking／签名回放、`reasoning_content`、think-tag 归一化和 Gemini `fileData`。[新版限制](../standalone/rust-proxy_CN.md)拒绝未支持的原生 Chat 内容块、缓存扩展和 Responses reasoning items。 | 为保留客户端建立字段／场景矩阵，保留可表达语义，明确拒绝不可表达的跨协议转换。OpenAI Chat 已有图片／音频类型字段，不能笼统说所有多模态都缺失；新增独立 Image/Audio/Video 操作另算范围。 |
...
| 2 | G02–G04：分类旧录制 fixture，按协议逐步补兼容。 | 每个输入标明保留、明确不支持或回退；同协议和跨协议预期分开。先补旧客户端需要的原生字段、推理／工具历史，再补媒体／缓存；验证 JSON、SSE、错误／截断响应和取消。 |
...
步骤 1 已完成，下一项推进步骤 2 的录制用例分类及原生协议保真。原生保真和密钥限制语义仍是发布阻塞项；后续每一步可能需要多份聚焦 PR。
