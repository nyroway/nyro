...
Anthropic 推理使用协议专有类型：`ChatRequest.anthropic_thinking` 携带 wire 配置，`ContentPart::AnthropicThinking`／`AnthropicRedactedThinking` 保留摘要、签名与加密数据；`Delta.anthropic_thinking` 用类型化 start／thinking／signature／stop 事件保留流式块边界，空摘要与连续推理块不会丢失。直接构造请求或 delta 时，无该能力则设为 `None`（delta 可使用 `Default`）。类型和有界流状态仍属于 `nyro-protocol`／`nyro-llm`，内核不参与推理协议。当前只支持严格 Anthropic 往返，其他协议目标以及工具调用后的推理块明确拒绝，不把厂商签名抽象成可互换字段。新协议默认行为与模型约束由上游决定，具体支持范围见[代理指南](../standalone/rust-proxy_CN.md#严格模式下的-anthropic-thinking)。

Gemini `generateContent` 推理配置使用 `ChatRequest.gemini_thinking`，沿用独立 Gemini wire 类型。`ContentPart::GeminiText`／`Delta.gemini_text` 保留文本 Part 上的 thought／签名及空文本边界；`ToolCall::Function.gemini`／`ToolCallDelta.gemini` 保存调用上的元数据和原始 ID 缺省信息，避免合成关联 ID 改写带签名 Part。直接构造这些节点时，无该能力则设置 `None`；枚举匹配需处理新增内容类型。能力位于既有协议与 LLM crate，默认值、模型支持与密码学校验仍由上游负责；其他协议拒绝，不能作为通用推理签名互换。具体支持边界见[代理指南](../standalone/rust-proxy_CN.md#严格模式下的-gemini-thinking)。

Responses 推理通过 `ChatRequest.responses_reasoning`（可选 boxed wire 配置）和 `responses_include_encrypted` 携带请求选项，通过 `ContentPart::ResponsesReasoning` 保留 item ID、摘要块、状态与不透明密文，通过 `Delta.responses_reasoning` 的类型化生命周期事件保留流式边界。配置默认值与模型限制由上游负责；解码器只校验可表达结构、顺序、快照一致性和资源界限。`output_item.added` 可含部分密文，最终回传值取自 `output_item.done`。无该能力时配置／delta 设为 `None`，include 标记设为 `false`。能力属于既有协议与 LLM crate，内核不参与；严格输出目前限于推理前缀、普通内容、函数调用。只有 effort-only 配置可与 OpenAI Chat 双向转换，其他控制及推理 items 对不兼容目标明确拒绝。详见[代理指南](../standalone/rust-proxy_CN.md#严格模式下的-responses-reasoning)。
