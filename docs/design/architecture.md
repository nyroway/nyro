...
有序 IR 的基础迁移使用 `Message.items`／`ResponseMessage.items: Vec<MessageItem>` 作为正文与调用的唯一顺序来源：`Content(Content)` 保留文本或嵌套内容块，`ToolCall(ToolCall)` 保留工具调用及协议元数据。原 `content`／`tool_calls` 字段移除；`content()`、`tool_calls()` 等仅为派生借用视图，使用前必须验证支持的排列。本轮 codec 仍限定最多一个正文组且位于调用前，直接构造多个正文组或交错项会被拒绝，不通过投影丢弃或移动节点。refusal／audio 的既有独立字段暂不调整。

`Delta` 使用 `role` 和 `events: Vec<PositionedDelta>`；每个事件由 `StreamPosition { item, part }` 与 `PartDelta` 组成。Anthropic 使用原始 block index；Responses 使用 output index 和 content／summary index；Gemini 使用接收 Part 的递增序号，不将跨帧 Part 自动认定为同一逻辑块。OpenAI Chat 使用 `OpenAiMessage` 和 `OpenAiTool(index)` 字段槽位，不假定跨字段总顺序；工具索引不按首次出现顺序重排。事件向量按输入顺序消费，签名与工具增量校验位置归属；`ChatChunk.usage`、choice finish 和全流 `Done` 保持原有职责。普通块的完整 start／end 生命周期、Responses 完整 item 元数据和新的交错排列将随各协议增量继续实现，本轮只迁移位置与现有推理生命周期，不据此宣布交错输出已支持。类型属于 `nyro-llm`，wire 类型继续属于 `nyro-protocol`，内核职责不变。回归见[有序 IR 测试](../../crates/nyro-llm/tests/ordered_ir_codec.rs)。

Anthropic 推理使用协议专有类型：`ChatRequest.anthropic_thinking` 携带 wire 配置，`ContentPart::AnthropicThinking`／`AnthropicRedactedThinking` 保留摘要、签名与加密数据；`Delta.events` 中的 `PartDelta::AnthropicThinking` 用类型化 start／thinking／signature／stop 事件保留流式块边界，空摘要与连续推理块不会丢失。请求未启用该能力时配置设为 `None`；无流式载荷时使用空 `events`（或 `Delta::default()`）。类型和有界流状态仍属于 `nyro-protocol`／`nyro-llm`，内核不参与推理协议。当前只支持严格 Anthropic 往返，其他协议目标以及工具调用后的推理块明确拒绝，不把厂商签名抽象成可互换字段。新协议默认行为与模型约束由上游决定，具体支持范围见[代理指南](../standalone/rust-proxy_CN.md#严格模式下的-anthropic-thinking)。
