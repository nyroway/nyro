...
Responses 推理通过 `ChatRequest.responses_reasoning`（可选 boxed wire 配置）和 `responses_include_encrypted` 携带请求选项，通过 `ContentPart::ResponsesReasoning` 保留 item ID、摘要块、状态与不透明密文，通过 `Delta.responses_reasoning` 的类型化生命周期事件保留流式边界。配置默认值与模型限制由上游负责；解码器只校验可表达结构、顺序、快照一致性和资源界限。`output_item.added` 可含部分密文，最终回传值取自 `output_item.done`。无该能力时配置／delta 设为 `None`，include 标记设为 `false`。能力属于既有协议与 LLM crate，其他协议目标明确拒绝，内核不参与；严格输出目前限于推理前缀、普通内容、函数调用。详见[代理指南](../standalone/rust-proxy_CN.md#严格模式下的-responses-reasoning)。
