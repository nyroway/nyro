...
包含图片或音频内容的对话仍然可以是 Chat。当前严格用户图片转换复用既有 `ContentPart::ImageUrl`，由 codec 映射 OpenAI Chat／Responses、Anthropic URL／base64 和 Gemini inlineData；图片来源解析为 `nyro-llm/codec` 私有辅助逻辑。工具结果中的图片同样属于 Chat：Anthropic／Responses 复用 `ContentPart::ImageUrl` 并保留结果内顺序与调用关联；Gemini 使用仅允许出现在工具结果中的 `ContentPart::GeminiFunctionResponse`，唯一保存 response 对象及有序媒体 parts，保留 `displayName`／引用。该专有结构不与普通文本／图片数组隐式互换；Chat Completions 工具消息仍只允许文本。图片不会进入内核，也不新增下载、转码服务或独立 Image 工作负载。图片生成、图片编辑、音频转写、语音合成等操作在需要时定义自己的请求和结果；不能仅按媒体名称制造装满可选字段的万能结构。如果未来的视频操作是异步任务，还应表达任务提交、状态与产物，而非强套普通同步响应。
...
有序 IR 的基础迁移使用 `Message.items`／`ResponseMessage.items: Vec<MessageItem>` 作为正文与调用的唯一顺序来源：`Content(Content)` 保留文本或嵌套内容块，`ToolCall(ToolCall)` 保留工具调用及协议元数据。原 `content`／`tool_calls` 字段移除；`content()`、`tool_calls()` 等仅为派生借用视图，使用前必须验证支持的排列。Anthropic、Gemini 和 Responses codec 按项遍历，支持正文、调用与各自专有推理的交错排列。Responses 的 `ResponsesMessage`／`ResponsesToolCall` 变体独立保存容器 ID／状态及正文／调用，区别于函数 `call_id`；借用视图识别这些变体。Chat 的静态投影仍限制最多一个调用前正文组，拒绝无法表达的排列。refusal／audio 的既有独立字段保留，Responses refusal 也可处于有序内容块中。

`Delta` 使用 `role` 和 `events: Vec<PositionedDelta>`；每个事件由 `StreamPosition { item, part }` 与 `PartDelta` 组成。Anthropic 使用原始 block index；Responses 使用 output index 和 content／summary index；Gemini 使用接收 Part 的递增序号，不将跨帧 Part 自动认定为同一逻辑块。OpenAI Chat 使用 `OpenAiMessage` 和 `OpenAiTool(index)` 字段槽位，不假定跨字段总顺序；工具索引不按首次出现顺序重排。事件向量按输入顺序消费，签名与工具增量校验位置归属；`ChatChunk.usage`、choice finish 和全流 `Done` 保持原有职责。普通正文／拒绝／调用使用 `PartDelta::Start(StreamPartKind)`／`End`，Responses 容器另有 `ResponsesItemStart`／`ResponsesItemEnd`，推理沿用专有生命周期；不向 Chat 字段槽位合成源端不存在的边界。Anthropic／Gemini encoder 用有界等待队列保持开始顺序，后项先完成时不能越过前项；未受阻的正文逐段输出。Responses 保留原始 item／part 坐标、ID、终态与完整终态快照。Chat 运行时使用有状态 encoder，在跨帧出现工具后正文等不可表达顺序时失败。类型属于 `nyro-llm`，wire 类型继续属于 `nyro-protocol`，内核职责不变。回归见[有序 IR 测试](../../crates/nyro-llm/tests/ordered_ir_codec.rs)。

Anthropic 推理使用协议专有类型：`ChatRequest.anthropic_thinking` 携带 wire 配置，`ContentPart::AnthropicThinking`／`AnthropicRedactedThinking` 保留摘要、签名与加密数据；`Delta.events` 中的 `PartDelta::AnthropicThinking` 用类型化 start／thinking／signature／stop 事件保留流式块边界，空摘要与连续推理块不会丢失。请求未启用该能力时配置设为 `None`；无流式载荷时使用空 `events`（或 `Delta::default()`）。类型和有界流状态仍属于 `nyro-protocol`／`nyro-llm`，内核不参与推理协议。当前只支持严格 Anthropic 往返，同协议支持工具调用后的推理块；其他协议目标明确拒绝，不把厂商签名抽象成可互换字段。新协议默认行为与模型约束由上游决定，具体支持范围见[代理指南](../standalone/rust-proxy_CN.md#严格模式下的-anthropic-thinking)。

Gemini `generateContent` 推理配置使用 `ChatRequest.gemini_thinking`，沿用独立 Gemini wire 类型。`ContentPart::GeminiText`／`PartDelta::GeminiText` 保留文本 Part 上的 thought／签名及空文本边界；`ToolCall::Function.gemini`／`ToolCallDelta.gemini` 保存调用上的元数据和原始 ID 缺省信息，避免合成关联 ID 改写带签名 Part。可选配置和调用元数据在未启用时设置 `None`；流式载荷放入带位置的 `Delta.events`，枚举匹配需处理对应类型。能力位于既有协议与 LLM crate，默认值、模型支持与密码学校验仍由上游负责；其他协议拒绝，不能作为通用推理签名互换。具体支持边界见[代理指南](../standalone/rust-proxy_CN.md#严格模式下的-gemini-thinking)。

Responses 推理通过 `ChatRequest.responses_reasoning`（可选 boxed wire 配置）和 `responses_include_encrypted` 携带请求选项，通过 `ContentPart::ResponsesReasoning` 保留 item ID、摘要块、状态与不透明密文，通过 `Delta.events` 中 `PartDelta::ResponsesReasoning` 的类型化生命周期事件保留流式边界。配置默认值与模型限制由上游负责；解码器只校验可表达结构、顺序、快照一致性和资源界限。`output_item.added` 可含部分密文，最终回传值取自 `output_item.done`。无该能力时配置设为 `None`，不产生该类流式事件，include 标记设为 `false`。能力属于既有协议与 LLM crate，内核不参与；严格输出支持推理、普通内容和函数调用的交错排列。只有 effort-only 配置可与 OpenAI Chat 双向转换，其他控制及推理 items 对不兼容目标明确拒绝。详见[代理指南](../standalone/rust-proxy_CN.md#严格模式下的-responses-reasoning)。

OpenAI Chat／Responses 共用 `nyro_protocol::openai::ReasoningEffort`；旧 `openai::responses::ReasoningEffort` 路径通过 re-export 保留。Chat wire 与 `OpenAiOptions.reasoning_effort` 从任意字符串改为可选枚举，直接调用方应使用枚举值；JSON 字段名和已支持取值不变，严格解析拒绝未知档位。两处直接 IR effort 冲突时拒绝，相同值合并；摘要、签名与加密状态仍使用原协议类型，不引入通用扩展字段或内核能力。新路径保留普通文本中的 `<think>` 字面量，兼容服务的 `reasoning_content` 扩展仅匹配原生模式保留。边界矩阵见[代理指南](../standalone/rust-proxy_CN.md#推理转换与旧行为取舍)。

协议字段、默认行为和约束以当前官方文档为准；旧代码用于核对部署行为和迁移差异，不作为语义正确性的标准。缓存控制按协议表达：OpenAI `prompt_cache_options` 的 mode／TTL 和兼容 retention 定义于 `nyro-protocol::openai`，由既有 `OpenAiOptions` 携带，Chat／Responses codec 分别编码；不把它们抽象成可与 Anthropic 缓存断点、Gemini 资源引用互换的通用 TTL。缺省字段由上游决定，Nyro 不注入默认值或断点。OpenAI 文本与用户图片断点由内容块上的可选类型化 `prompt_cache_breakpoint` 携带，Chat／Responses 严格转换保留位置、顺序及工具关联，TTL 仍属于请求选项；其他协议目标明确拒绝，生成结果不能混入输入控制。直接构造 `ContentPart::Text`／`ImageUrl` 的消费者需提供该字段，无断点时设为 `None`。Anthropic `cache_control` 由请求、内容块、工具定义／调用及外层工具结果各自的可选 `anthropic_cache_control` 携带，TTL 使用独立的 Anthropic wire 类型；严格 Anthropic 转换保留，其他协议拒绝。system 文本块不再拼接，避免改变断点位置；工具定义由 `ir::Tool` 承载，OpenAI wire 类型不加入 Anthropic 字段。直接构造这些 IR 节点时，无控制则设为 `None`。Anthropic 请求／生成内容使用独立 wire 类型，避免输入控制进入 JSON／SSE 响应。Gemini cachedContent 继续只在匹配的原生路径保留。控制字段与用量支持范围分别验收；当前同时支持 OpenAI `cache_write_tokens`，复用 `Usage.cache_creation` 且不推测 TTL。读取与写入之和不得超过包含缓存的输入总量；原生路径保留原文并校验已知子集。新官方字段未实现时应作为明确的支持边界记录，不能据旧类型断言协议本身无法表达。
...
根程序通过 `nyro_llm::runtime::SharedResources` 显式共享路由历史、健康、模型 rate、主体请求窗口和 quota 注册表。quota 已消耗及在途账本在模型移除／加回后继续保留；有活跃绑定或已消费余额时修改规则会拒绝候选构建，避免配置变更清零。进程重启会重置进程内状态。quota 超限返回原生协议 `429`，不带 `Retry-After` 并立即释放并发许可；其之前的 rate 准入不退款。计量、协议映射和资源组合均未进入 `nyro-kernel`。
...
当前实现由 [`nyro-control`](../../crates/nyro-control/src/lib.rs) 拥有类型化快照、严格 schema 校验和专用 SQLite／PostgreSQL 单行存储，其 [`entity`](../../crates/nyro-control/src/entity.rs) 模块拥有实体编辑命令、凭证更新、引用检查和脱敏投影；复用 `nyro-config` 快照、`nyro-llm` 配置类型和 `nyro-security` 密钥类型，不负责 HTTP 或 Host 激活。根 [`src/control.rs`](../../src/control.rs) 负责管理认证、服务持有队列和发布，[`src/control/entity.rs`](../../src/control/entity.rs) 仅适配 HTTP，[`src/serve.rs`](../../src/serve.rs) 负责启动、独立回环管理监听及退出。`nyro-kernel` 和 `nyro-llm` 不依赖控制面或数据库。

发布先校验进程设置并构建候选，再提交数据库已发布快照，最后激活 Host；不宣称数据库与代际跨资源原子提交。提交前拒绝保持旧发布目标及活动代际；提交后激活中断返回 `202 pending`，重启恢复持久化目标。API 区分草稿、已发布和活动版本，重复发布等价快照不新建代际。接受后的写操作由服务持有，客户端断连不取消它，调用方需用 GET 核对结果。详情见[服务指南](../standalone/rust-serve_CN.md)。

`GET /admin/config` 及 Provider／Model／API Key 列表和详情均为脱敏投影，凭证只表示为 `has_*`，整个代理 URL 不公开。显式 `GET /admin/config/export` 保留原完整配置响应及明文凭证，沿用管理认证与 `no-store`；完整配置编辑从导出开始，不能将读取投影直接写回。文件／SQLite／PostgreSQL 仍保存原始凭证。实体编辑与完整配置 PUT 共用全局草稿版本和完整校验，元数据完整替换，凭证通过 Keep／Set／Clear 显式更新；省略凭证保留旧值。引用中的 Provider／API Key 删除被拒绝，不级联；编辑不自动发布。

存储只持久化配置，限制计数、健康和路由历史仍在内存。两个后端均为专用数据库、单进程所有权，不导入旧库。SQLite 使用独占连接；PostgreSQL 使用单个非池化连接与会话 advisory lock，固定 `public.nyro_control_state` v1，不提供自动重连或多副本协调。断连／存储超时使控制存储停用，当前数据面代际继续服务；写入结果无法确认时返回 `503 storage_outcome_unknown`，不能保证回滚，需重启后读取持久化状态核对。WebUI、OAuth 和持久化观测仍待完成。

当前复用已有 crate，公共存储门面位于 [`storage/mod.rs`](../../crates/nyro-control/src/storage/mod.rs)，私有后端位于 [`storage/sqlite.rs`](../../crates/nyro-control/src/storage/sqlite.rs) 与 [`storage/postgres.rs`](../../crates/nyro-control/src/storage/postgres.rs)。完整快照、版本比较、实体编辑和发布契约保持一致，无需提前引入通用仓储框架。下表及 10.2 的进一步目录拆分为目标划分。

| 方式 | 所在位置 | 语义 |
|---|---|---|
| file / standalone | `nyro-config/src/source/file.rs` | 文件解析、校验、构建不可变配置快照 |
| SQLite | `nyro-control/src/storage/sqlite.rs` | 配置快照的持久化与事务，单进程独占 |
| Postgres | `nyro-control/src/storage/postgres.rs` | 与 SQLite 对齐的快照契约，单连接会话所有权 |
...
以下分层是管理能力扩展后的目标，当前两个私有后端各自持有单连接；尚未使用连接池或建立这些子目录。
...
目标中根装配层选择后端并持有连接资源；若引入连接池，使用者持有连接池引用。候选重建不能关闭仍在使用的共享池。若后续限制模块确实需要自己的 SQL 状态，其查询和 schema 归 `nyro-limit`；只有实际出现共用连接代码时才考虑提取轻量数据库包，不能让限制模块反向依赖控制面。
...
| 控制面、存储、管理与持久化观测 | G10 部分完成：SQLite／PostgreSQL 完整草稿、Provider／Model／API Key CRUD、脱敏查询与显式导出、管理认证、版本冲突、显式发布和重启恢复；WebUI、OAuth、旧数据导入、持久化预算和观测尚未完成 | G10–G12 |
...
| `src-server/`、`nyro-server --mode ...` | 继续承担现有发布；根 `src/` 已实现实验性文件配置 `nyro proxy` 和专用 SQLite／PostgreSQL `nyro serve`，`nyro-control` 承接快照存储、实体编辑和脱敏投影 |
