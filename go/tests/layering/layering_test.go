...
func TestCompositionRootImportPolicy(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name     string
		importer string
		...
		{"runtime cannot import bootstrap", "internal/llm/runtime", "internal/bootstrap", false},
		{"admin cannot import bootstrap child", "internal/admin", "internal/bootstrap/catalog", false},
		{"runtime cannot import integration", "internal/llm/runtime", "internal/integration", false},
		{"bootstrap may import its own child", "internal/bootstrap", "internal/bootstrap/catalog", true},
		{"bootstrap child may import bootstrap sibling", "internal/bootstrap/runtime", "internal/bootstrap/catalog", true},
		{"integration may import its own child", "internal/integration", "internal/integration/mcp", true},
		{"bootstrap cannot import integration", "internal/bootstrap", "internal/integration", false},
		{"integration cannot import bootstrap", "internal/integration", "internal/bootstrap", false},
		{"ordinary internal edge is unaffected", "internal/llm/runtime", "internal/llm/protocol", true},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if got := compositionRootImportAllowed(test.importer, test.imported); got != test.want {
				t.Fatalf("compositionRootImportAllowed(%q, %q) = %v, want %v", test.importer, test.imported, got, test.want)
			}
		})
	}
}
...
func compositionRootImportAllowed(importer, imported string) bool {
	for _, root := range []string{"internal/bootstrap", "internal/integration"} {
		if packageWithin(imported, root) {
			return packageWithin(importer, root)
		}
	}
	return true
}

var canonicalGeneratedMarker = regexp.MustCompile(`^// Code generated .* DO NOT EDIT\.$`)

func hasCanonicalGeneratedMarker(source []byte) bool {
	line, _, _ := bytes.Cut(source, []byte("\n"))
	line = bytes.TrimSuffix(line, []byte("\r"))
	return canonicalGeneratedMarker.Match(line)
}

func blankImportAllowed(file, importPath string) bool {
	return file == "internal/platform/database/sqlite/sqlite.go" &&
...
func TestCompositionRootsAreNotImportedByDomainPackages(t *testing.T) {
	t.Parallel()
	for _, pkg := range loadInternalPackages(t) {
		for _, imp := range pkg.imports {
			if !compositionRootImportAllowed(pkg.name, imp) {
				t.Errorf("composition-root boundary: %s imports %s", pkg.name, imp)
			}
		}
	}
}
...
func TestNoHandwrittenRegistrationInitOrUnapprovedBlankImports(t *testing.T) {
	t.Parallel()
	root := moduleRoot(t)
	err := walkHandwrittenProductionGo(root, func(_ string, rel string, _ []byte, file *ast.File) error {
		...
		for _, imported := range file.Imports {
			...
			if !blankImportAllowed(rel, importPath) {
				t.Errorf("implicit registration: %s has unapproved blank import %s", rel, importPath)
			}
		}
		return nil
	})
...
func TestCanonicalGeneratedMarkerPolicy(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name   string
		source string
		...
		{"canonical protoc marker", "// Code generated by protoc-gen-go. DO NOT EDIT.\npackage generated\n", true},
		{"marker after package", "package handwritten\n// Code generated by a liar. DO NOT EDIT.\n", false},
		{"separated substrings", "// Code generated eventually\n// DO NOT EDIT\npackage handwritten\n", false},
		{"missing canonical period", "// Code generated by a liar. DO NOT EDIT\npackage handwritten\n", false},
		{"leading whitespace", " // Code generated by a liar. DO NOT EDIT.\npackage handwritten\n", false},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if got := hasCanonicalGeneratedMarker([]byte(test.source)); got != test.want {
				t.Fatalf("hasCanonicalGeneratedMarker() = %v, want %v", got, test.want)
			}
		})
	}
}

func TestBlankImportPolicy(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name       string
		file       string
		...
		{"reviewed sqlite driver", "internal/platform/database/sqlite/sqlite.go", "github.com/glebarez/go-sqlite", true},
		{"sqlite driver elsewhere", "internal/platform/database/sqlite/other.go", "github.com/glebarez/go-sqlite", false},
		{"different driver in reviewed file", "internal/platform/database/sqlite/sqlite.go", "example.com/driver", false},
		{"internal registration", "internal/platform/database/sqlite/sqlite.go", modulePath + "/internal/bootstrap", false},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if got := blankImportAllowed(test.file, test.importPath); got != test.want {
				t.Fatalf("blankImportAllowed(%q, %q) = %v, want %v", test.file, test.importPath, got, test.want)
			}
		})
	}
}
...
func walkHandwrittenProductionGo(root string, visit func(path, rel string, raw []byte, file *ast.File) error) error {
	for _, sourceRoot := range []string{"internal", "cmd"} {
		err := filepath.WalkDir(filepath.Join(root, sourceRoot), func(path string, entry os.DirEntry, walkErr error) error {
			...
			if hasCanonicalGeneratedMarker(raw) {
				return nil
			}
			rel, err := filepath.Rel(root, path)
			if err != nil {
