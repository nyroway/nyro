...
func TestConsumerUpdateReplacesQuotasWholesale(t *testing.T) {
	b := newTestBackend(t)
	var s storage.Storage = b
	...
	// nil Quotas on a second update must leave quotas unchanged.
	updated2, err := s.Consumers().Update(consumer.ID, storage.UpdateConsumer{Name: strPtr("acme2")})
	if err != nil {
	...
	// Empty (non-nil) slice clears quotas.
...
func TestConsumerUpdateRejectsInvalidQuota(t *testing.T) {
	b := newTestBackend(t)
	var s storage.Storage = b
...
func TestConsumerUpdateReplacesRoutesByModelName(t *testing.T) {
	b := newTestBackend(t)
	var s storage.Storage = b
	...
	// Unknown model name must error and list the unknown name.
	...
	// Prior successful state must be untouched by the failed update.
	got, err := s.Consumers().Get(consumer.ID)
