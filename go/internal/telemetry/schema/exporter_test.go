...
func TestIsExporterSettingKey(t *testing.T) {
	tests := map[string]bool{
		"obs_logs_exporter":                     true,
		"obs_metrics_prometheus_path":           true,
		"obs_traces_otlp_endpoint":              true,
		"obs_logs_retention_days":               false,
		"proxy.request_timeout":                 false,
		"obs_metrics_unknown_exporter_endpoint": false,
	}
	for key, want := range tests {
		if got := IsExporterSettingKey(key); got != want {
			t.Errorf("IsExporterSettingKey(%q) = %v, want %v", key, got, want)
		}
	}
}
