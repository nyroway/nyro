...
func TestSelectPriority(t *testing.T) {
	r := New()
	backends := []storage.RouteUpstream{
...
func TestSelectSkipsCooldown(t *testing.T) {
	r := New()
	b1 := storage.RouteUpstream{UpstreamID: "p1", Model: "a"}
	b2 := storage.RouteUpstream{UpstreamID: "p2", Model: "b"}
	...
	ordered := r.Select([]storage.RouteUpstream{b1, b2}, storage.BalancePriority)
	...
	all := r.Select([]storage.RouteUpstream{b1, b2}, storage.BalancePriority)
	if len(all) != 2 {
...
func TestRecordLatencyEMA(t *testing.T) {
	r := New()
	k := KeyOf(storage.RouteUpstream{UpstreamID: "p", Model: "m"})
	r.Record(k, true, 100)
