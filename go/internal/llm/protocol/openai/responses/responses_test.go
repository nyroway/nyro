...
func TestConstructorsExposeResponsesEndpoint(t *testing.T) {
	t.Parallel()
	if got := NewIngress().Endpoint(); got != protocol.OpenAIResponsesV1 {
		t.Errorf("ingress endpoint = %v", got)
	}
	if got := NewEgress().Endpoint(); got != protocol.OpenAIResponsesV1 {
