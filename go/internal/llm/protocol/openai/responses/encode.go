...
// requestEncoder implements protocol.RequestEncoder for the Responses API.
...
func (requestEncoder) Encode(req *llm.ChatRequest) (protocol.WireRequest, error) {
	...
	if len(input) == 0 {
		return protocol.WireRequest{}, errors.New("responses request requires at least one input item")
	}
