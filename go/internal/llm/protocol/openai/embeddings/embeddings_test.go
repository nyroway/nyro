...
func TestConstructorsExposeEmbeddingsEndpoint(t *testing.T) {
	t.Parallel()
	if got := NewIngress().Endpoint(); got != protocol.OpenAIEmbeddingsV1 {
		t.Errorf("ingress endpoint = %v", got)
	}
	if got := NewEgress().Endpoint(); got != protocol.OpenAIEmbeddingsV1 {
