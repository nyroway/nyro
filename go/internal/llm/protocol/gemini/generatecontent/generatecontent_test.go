...
func TestConstructorsExposeGeminiEndpoint(t *testing.T) {
	t.Parallel()
	if got := NewIngress().Endpoint(); got != protocol.GeminiGenerateContentV1Beta {
		t.Errorf("ingress endpoint = %v", got)
	}
	if got := NewEgress().Endpoint(); got != protocol.GeminiGenerateContentV1Beta {
...
func TestStreamEncode(t *testing.T) {
	...
	all := append(append([]protocol.Event{}, frames...), done...)
